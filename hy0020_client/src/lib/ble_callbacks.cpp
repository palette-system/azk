...
// BLE クライアント(左手キーボード接続用)
BLEClientDis  clientDis;  // シリアルクライアント
...
void HidrawCallbackExec(int data_length) {
	...
	switch (*command_id) {
		...
		case id_ble_uart_list_start: {
			...
			check_addr = (uint8_t *)malloc(20 * BLE_GAP_ADDR_LEN);
  ...
  if (clientDis.discover(conn_handle)) {
    ...
    if (clientDis.getModel(model_buf, sizeof(model_buf))) {
		sprintf(json_buf, "{\"addr\":[%d,%d,%d,%d,%d,%d],\"name\":\"%s\"}",
          addr.addr[0], addr.addr[1], addr.addr[2], addr.addr[3], addr.addr[4], addr.addr[5], model_buf);
